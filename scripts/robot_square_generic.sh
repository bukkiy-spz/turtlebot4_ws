...
selected_cmd_vel_topic="/${robot_name}/cmd_vel"
selected_subscriber_hint="create3_repub"
selected_reliability="reliable"
selected_control_mode="cmd_vel"
selected_drive_distance_action="/${robot_name}/drive_distance"
selected_rotate_angle_action="/${robot_name}/rotate_angle"
...
detect_cmd_vel_topic() {
  ...
  if grep -qx "/${robot_name}/cmd_vel_nav" <<<"${topic_list}"; then
    info_output="$(ros2 topic info -v /${robot_name}/cmd_vel_nav 2>/dev/null || true)"
    if grep -q "Node name: velocity_smoother" <<<"${info_output}" || \
      ros2 node list 2>/dev/null | grep -qx "/${robot_name}/velocity_smoother"; then
      selected_cmd_vel_topic="/${robot_name}/cmd_vel_nav"
...
elif [[ "${selected_cmd_vel_topic}" == "/${robot_name}/cmd_vel_nav" ]]; then
